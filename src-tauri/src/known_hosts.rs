...
//!
//! Ported from the hopline sibling project (issue #11 asks to reuse its
//! working session/auth code rather than reinventing it). The file lives
//! next to `connections.json` in the app config dir (not `~/.ssh/known_hosts`,
//! so we never touch the user's real SSH state and stay testable via a
//! tempdir).
//!
...
fn describe_keys_error(e: russh::keys::Error) -> String {
    match e {
        russh::keys::Error::IO(_) => {
            "could not read or write pgcove's known_hosts file".to_string()
        }
...
pub fn trust(dir: &Path, host: &str, port: u16, key: &PublicKey) -> Result<(), String> {
    ...
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)
            .map_err(|_| "could not create pgcove's config directory".to_string())?;
    }
